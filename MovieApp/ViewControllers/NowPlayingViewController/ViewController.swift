...
class ViewController: mainViewController,UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        realmDB.shared.delegate_NowPlaying = self
        realmDB.shared.read()
        getMovies()
        ...
        scroll.cr.addHeadRefresh(animator: NormalHeaderAnimator()){
            ...
            Timer.scheduledTimer(withTimeInterval: 0.5, repeats: false) { (Timer) in
                self.getMovies(showIndicator:false)
            }
    ...
    func getMovies(showIndicator:Bool = true){
        moreDataExists = false
        if showIndicator{
            showActivityIndicator()
        }
        nowPlayingControllerRequests.shared.getNowPlayingMovies(page: page) { (results) in
            self.page += 1
            self.drawMovies(allMovies: results)
...
extension ViewController:handleSavedMoviesIcon_NowPlaying{
